...
describe('pdf page editors', () => {
  it('loads pages and exercises split, extract, rotate, organize and merge flows', async () => {
    ...
    const mr = vi.fn(), rm = vi.fn(), po = vi.fn(); render(<MergeEditor files={[file('a.pdf'), file('b.pdf')]} onReorder={mr} onRemove={rm} onPageOrderChange={po} />); const name = await screen.findByText('a.pdf'); expect(name).toBeInTheDocument(); fireEvent.click(screen.getByRole('switch')); await screen.findByText(/Reorder individual pages/); if (screen.queryAllByTitle('Remove Page')[0]) await user.click(screen.queryAllByTitle('Remove Page')[0]);
  });
});
